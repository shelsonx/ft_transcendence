...
class SignUpUseCaseTests(TestCase):
    ...
    def setUp(self, login_type_repository_mock, user_repository_mock, two_factor_service_mock):
    ...
    async def test_sign_up_usecase_return_two_factor_code_sent_to_email(self, execute_mock):
        password = "123456"
        ...
        self.user_repository_mock.get_user_by_email = AsyncMock(side_effect=ObjectDoesNotExist)
        ...
        result = await self.sign_in_usecase.execute(SignUpDto(email=user.email, password=password, user_name=user.user_name))
    ...
    async def test_sign_up_usecase_return_failed_email_already_exists(self):
        ...
        with self.assertRaises(FieldAlreadyExistsException):
            await self.sign_in_usecase.execute(SignUpDto(email=user.email, password=password, user_name=user.user_name))

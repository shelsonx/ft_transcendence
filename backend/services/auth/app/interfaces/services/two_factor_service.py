...
class ITwoFactorService(ABC):
    ...
    def __init__(self, two_factor_repository: ITwoFactorGameRepository):
    ...
    async def validate_and_delete_two_factor(self, two_factor: ValidateGame2FactorCodeDto) -> bool:
        is_valid = await self.validate_code(two_factor)
        if is_valid:
            await self.delete_two_factor(user_id)
        return is_valid

    async def delete_two_factor(self, user_id: str) -> None:
        await self.two_factor_repository.delete_two_factor_by_game_details(user_id)
